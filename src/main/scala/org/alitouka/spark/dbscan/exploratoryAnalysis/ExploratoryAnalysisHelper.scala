...
object ExploratoryAnalysisHelper {
  ...
  /** Calculates number of point's neighbors within a specified distance and builds a histogram
    ...
    * @param distanceMeasureSuite Distance measure class
    */
  def countNumberOfPointsWithinDistance (
      data: PointsPartitionedByRegionRDD,
      eps: Double,
      outputPath: String,
      numberOfBuckets: Int = DefaultNumberOfBucketsInHistogram,
      distanceMeasureSuite: DistanceMeasureSuite = DbscanSettings.getDefaultDistanceMeasureSuite): Unit = {

    val clock = new Clock ()

    val settings = new DbscanSettings()
      .withEpsilon(eps)
      .withDistanceMeasureSuite(distanceMeasureSuite)
    ...
    val indexedPoints = PointsPartitionedByRegionRDD.extractPointIdsAndCoordinates (data)
