...
import org.alitouka.spark.dbscan.spatial.rdd.{PointsPartitionedByRegionRDD, PartitioningSettings}
...
object DistanceToNearestNeighborDriver extends DistanceCalculation {
  ...
  def main (args: Array[String]) {
    ...
    if (argsParser.parse(args)) {
      ...
      val settings = new DbscanSettings().withDistanceMeasureSuite(argsParser.args.distanceMeasureSuite)
  ...
  def createNearestNeighborHistogram(
    ...
    val partitionedData = PointsPartitionedByRegionRDD(data, partitioningSettings)
    
    val pointIdsWithDistances = partitionedData.mapPartitions {
      it =>
        {
          calculateDistancesToNearestNeighbors(it, settings.distanceMeasureSuite.distanceMeasure)
        }
    }
