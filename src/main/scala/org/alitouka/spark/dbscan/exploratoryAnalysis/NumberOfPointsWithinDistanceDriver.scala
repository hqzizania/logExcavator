...
import org.alitouka.spark.dbscan.{PointCoordinates, PointId, DbscanSettings, RawDataSet}
...
import org.alitouka.spark.dbscan.spatial.rdd.{PartitioningSettings, PointsPartitionedByRegionRDD}
...
object NumberOfPointsWithinDistanceDriver {
  ...
  def main (args: Array[String]) {
    ...
    if (argsParser.parse(args)) {
      ...
      val settings = new DbscanSettings()
        ...
        .withDistanceMeasureSuite(argsParser.args.distanceMeasureSuite)
  ...
  def createNumberOfPointsWithinDistanceHistogram(
    ...
    val partitionedData = PointsPartitionedByRegionRDD (data, partitioningSettings, settings)
    ...
    val indexedPoints = PointsPartitionedByRegionRDD.extractPointIdsAndCoordinates (partitionedData)
