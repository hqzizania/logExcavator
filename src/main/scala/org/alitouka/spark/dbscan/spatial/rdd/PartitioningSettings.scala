...
/** Represents density-based partitioning settings. Most of the time you can use default settings,
  ...
  *                                      This parameter represents the number of splits at each level
  * @param numberOfLevelsWithinPartition Each partition will be split into parts to build an indexing data structure.
  *                                      This parameter represents the number of levels.
  *                                      
  */
class PartitioningSettings  (
     ...
     val numberOfLevelsWithinPartition: Int = PartitioningSettings.DefaultNumberOfLevelsWithinPartition)
  extends Serializable {

  def withNumberOfSplits (n: Int): PartitioningSettings = {
    new PartitioningSettings(n, this.numberOfLevels, this.numberOfPointsInBox, this.numberOfSplitsWithinPartition)
  }
  ...
  def withNumberOfPointsInBox (np: Int): PartitioningSettings = {
    new PartitioningSettings(this.numberOfSplits, this.numberOfLevels, np, this.numberOfSplitsWithinPartition)
  }
}
...
object PartitioningSettings {
  ...
  def DefaultNumberOfSplitsWithinPartition = 2
  
  def DefaultNumberOfLevelsWithinPartition = 10
