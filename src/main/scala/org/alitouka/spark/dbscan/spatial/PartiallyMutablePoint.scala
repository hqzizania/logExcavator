...
private [dbscan] class PartiallyMutablePoint (p: Point, val tempId: TempPointId) extends Point (p) {
  ...
  def toImmutablePoint: Point = new Point (this.coordinates, this.pointId, this.regionId, this.distanceFromOrigin,
    this.precomputedNumberOfNeighbors, this.transientClusterId, this.originId)
