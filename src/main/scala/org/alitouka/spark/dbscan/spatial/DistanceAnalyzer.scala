...
private [dbscan] class DistanceAnalyzer (
  ...
  implicit val distanceMeasure: DistanceMeasure = settings.distanceMeasureSuite.distanceMeasure

  def this () = this (new DbscanSettings ())

  def countNeighborsForEachPoint ( data: PointsPartitionedByRegionRDD )
    ...
    val partitionedAndSortedCounts = new ShuffledRDD [PointSortKey, Long, Long] (
      allPointCounts,
      new GenericPartitioner (data.boxes)
  ...
  def countClosePointsWithinEachBox (data: PointsPartitionedByRegionRDD)
    :RDD[(PointSortKey, Long)] = {
  ...
  def countClosePointsWithinPartition (it: Iterator[(PointSortKey, Point)], boundingBox: Region): Iterator[(PointSortKey, Long)] = {
    ...
    //val partitionIndex = new PartitionIndex (boundingBox.asInstanceOf[Box], settings, partitioningSettings)
  ...
  def findPointsCloseToBoxBounds [U <: RDD[(PointSortKey, Point)]] (data: U, boxes: Iterable[Region], eps: Double)
    :RDD[Point] = {
      ...
      (idx, it) => {
        ...
        for (i <- 1 until pointsInPartition.length) {
          ...
          while (j >= 0 && pi.distanceFromOrigin - pointsInPartition(j).distanceFromOrigin <= eps) {
            ...
            if (pi.regionId != pj.regionId && calculateDistance(pi, pj) <= settings.epsilon) {
