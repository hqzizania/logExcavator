...
import org.alitouka.spark.dbscan.spatial.rdd.{PointsInAdjacentRegionsRDD, PartitioningSettings, PointsPartitionedByRegionRDD}
import scala.Some
...
class DistributedDbscan (
  ...
  private [dbscan] implicit val distanceMeasure: DistanceMeasure = settings.distanceMeasureSuite.distanceMeasure
  ...
  override protected def run(data: RawDataSet): DbscanModel = {
    ...
    DebugHelper.doAndSaveResult(data.sparkContext, "neighborsCount") {
      path => {

        pointsWithNeighborCounts.mapPartitionsWithIndex {
          (idx, it) => {
            it.map ( x => x._2.coordinates.mkString (",") + "\t" + x._2.precomputedNumberOfNeighbors)
          }
        }.saveAsTextFile(path)
      }
    }
    ...
    val partiallyClusteredData: RDD[(PointSortKey, Point)] = pointsWithNeighborCounts.mapPartitionsWithIndex (
      (partitionIndex, it) => {
  ...
  /** Finds clusters within one partition
    ...
    *
    * @param boundingBox A [[org.alitouka.spark.dbscan.spatial.Region]] object which represents bounds of the whole dataset
  ...
  /** Expands a cluster. In contrast to the function described in http://en.wikipedia.org/wiki/DBSCAN ,
    ...
    */
  private def expandCluster (points: Map[TempPointId, PartiallyMutablePoint], index: RegionPartitionIndex,
   ...
   */
  private def findUnvisitedNeighbors (index: RegionPartitionIndex, pt: PartiallyMutablePoint, settings: DbscanSettings)
  ...
  private def mergeClustersFromDifferentPartitions (partiallyClusteredData: RDD[(PointSortKey, Point)],
    boxes: Iterable[Region]): RDD[Point] = {
  ...
  private def generateMappings (pointsCloseToBoxBounds: RDD[Point], boxes: Iterable[Region])
    :(HashSet[(HashSet[ClusterId], ClusterId)], Map [PointId, ClusterId])  = {

    val pointsInAdjacentBoxes = PointsInAdjacentRegionsRDD (pointsCloseToBoxBounds, boxes)
      ...
      (idx, it) => {
        ...
        for (i <- 1 until pointsInPartition.length) {
          ...
          while (j >= 0 && pi.distanceFromOrigin - pointsInPartition(j).distanceFromOrigin <= settings.epsilon) {
            ...
            if (pi.regionId != pj.regionId && pi.clusterId != pj.clusterId && calculateDistance(pi, pj) <= settings.epsilon) {
